...
def take_images_and_return_corrections(cam, camera_params, detector, cam_mtx, cam_dist):
    measured_positions = []
    measured_quats = []
    show_message_and_wait('Put the block on position')
    # create corrections dict with init transformations because we don't have corrections and want to have raw poses
    corrections = {i: {"pos_corr": np.array([0, 0, 0]), "quat_corr": np.array([1, 0, 0, 0])} for i in range(108)}
    for i in range(images_num):
        ...
        mtx = get_tag_poses_from_image(im, target_tag_ids, target_tag_size, ref_tag_id,
                                       ref_tag_size, ref_tag_pos, camera_params,
                                       corrections, detector, cam_mtx, cam_dist)
    ...
    # TODO: do quaternion averaging right (using average_quaternions)
...
def collect_poses(cam, camera_params, detector, cam_mtx, cam_dist, fname, n):
    for i in range(n):
        measured_pos, measured_quat, tag_id, pos_std, quat_std = take_images_and_return_corrections(cam,
                                                                                                    camera_params,
                                                                                                    detector,
                                                                                                    cam_mtx,
                                                                                                    cam_dist)
...
def create_file_with_corrections(block_sizes, measured_poses, filename):
    ...
    for id in measured_poses:
        ...
        measured_quat = Quaternion(measured_poses[id]['quat'])
...
if __name__ == "__main__":
    ...
    # define block sizes
    with open('../data/block_sizes.json', mode='r') as f:
        block_sizes = json.load(f)

    # take textures and write measured poses to file
    tags_number = 108  # max tag id
    collect_poses(cam, camera_params, detector, cam1_mtx, cam1_dist, '../data/measured_poses2.json', tags_number)

    # compute and save corrections
    measured_poses = read_measured_poses('../data/measured_poses2.json')
    create_file_with_corrections(block_sizes, measured_poses, '../data/corrections2.json')
